...
//轮播图
var sliderTimer = null; //轮播图控制器
var slider = document.getElementById('slider');
var slides = slider.querySelector('.slides');
var sList = slides.querySelectorAll('.slide');
var cursor = slider.querySelector('#slider-cursor');
var bullets = cursor.querySelectorAll('span');
...
function pageLoad() {
	...
	// 设置轮播图
	sliderTimer = setTimeout('setSlider()',5000);
...
function eventListener() {
	...
	//轮播图鼠标事件
	Unit.addEvent(slider,'mouseover',function(){
		clearTimeout(sliderTimer);
	});
	Unit.addEvent(slider,'mouseout',function(){
		sliderTimer = setTimeout('setSlider()',5000);
	});

	//轮播图分页器

	Unit.addEvent(cursor,'click',function(event){
		var e = window.event || event;
		var element = e.srcElement ? e.srcElement : e.target;
